...
if (failed.length) {
  console.warn(
    `⚠ ${failed.length}/${haps.length} voices errored — first:`,
    String(failed[0].reason).slice(0, 140),
  );
}
...
console.log(
  `✓ ${OUT}  ·  ${DURATION_SECONDS.toFixed(2)}s @ ${BPM} BPM  ·  peak ${peak.toFixed(2)} → -1 dBFS  ·  ${haps.length} voices`,
);
console.log(
  `  Composition is ${DURATION_IN_FRAMES} frames — src/Root.tsx reads this from sync.mjs automatically.`,
);
