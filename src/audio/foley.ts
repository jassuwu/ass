 ...
 */
export function playSlap(g: AudioGraph, power01: number): void {
  const { ctx, dry, wet } = g;
  ...
  crack.buffer = noiseBuffer(ctx, 0.06);
  const bp = ctx.createBiquadFilter();
  bp.type = "bandpass";
  bp.frequency.value = (750 + 1700 * p) * jitter;
  bp.Q.value = 0.8;
  ...
  thump.type = "sine";
  thump.frequency.setValueAtTime((90 + 35 * p) * jitter, t);
  thump.frequency.exponentialRampToValueAtTime(44, t + 0.14);
  const thumpGain = ctx.createGain();
  thumpGain.gain.setValueAtTime(0.22 + 0.34 * p, t);
  thumpGain.gain.exponentialRampToValueAtTime(0.001, t + 0.24);
  ...
  knock.type = "triangle";
  knock.frequency.value = 175 * jitter;
  const knockGain = ctx.createGain();
  knockGain.gain.setValueAtTime(0.05 + 0.09 * p, t);
  knockGain.gain.exponentialRampToValueAtTime(0.001, t + 0.09);
